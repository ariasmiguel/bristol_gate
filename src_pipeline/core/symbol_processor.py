...
class SymbolProcessor:
    ...
    def prepare_symbols_for_fetch(self, 
        ...
        prepared_symbols = self._add_default_dates(prepared_symbols, default_start_date)
    ...
    def _filter_by_source(self, symbols_df: pd.DataFrame, source_type: str) -> pd.DataFrame:
        ...
        source_column_candidates = [
            'source', 'string.source', 'Source', 'data_source'
        ]
    ...
    def _standardize_column_names(self, symbols_df: pd.DataFrame) -> pd.DataFrame:
        ...
        column_mapping = {
            ...
            'Symbol': 'string.symbol', 
            'series_id': 'string.symbol',
            'source': 'string.source',
            'Source': 'string.source',
            'data_source': 'string.source',
            'start_date': 'date.series.start',
            'series_start': 'date.series.start',
            'start': 'date.series.start'
        }
    ...
    def _add_default_dates(self, symbols_df: pd.DataFrame, 
        ...
        if 'date.series.start' not in df_with_dates.columns:
            df_with_dates['date.series.start'] = default_start_date
            self.logger.debug(f"Added default start date: {default_start_date}")
        else:
            # Fill missing values with default
            missing_count = df_with_dates['date.series.start'].isna().sum()
            if missing_count > 0:
                df_with_dates['date.series.start'] = df_with_dates['date.series.start'].fillna(default_start_date)
    ...
    def _validate_symbol_format(self, symbols_df: pd.DataFrame, 
        ...
        """
        required_columns = ['string.symbol', 'string.source', 'date.series.start']
        ...
        validated_df = symbols_df.dropna(subset=['string.symbol']).copy()
    ...
    def _validate_date_format(self, symbols_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            test_dates = pd.to_datetime(df_validated['date.series.start'])
            
            # Convert back to string format (YYYY-MM-DD)
            df_validated['date.series.start'] = test_dates.dt.strftime('%Y-%m-%d')
